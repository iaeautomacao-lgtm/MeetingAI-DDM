...
- **2026-09-17** — **Reunião com múltiplos setores.** Antes: `Setor` era um `<select>` de escolha única na Nova Reunião, e a edição tinha um campo separado `Compartilhar com setores` (textarea de texto livre, fácil de digitar errado). Agora os dois modais usam o mesmo componente de chips multi-seleção (`.setor-multi` / `.setor-chip`), a textarea foi removida e o setor principal é o primeiro marcado (chip laranja com a etiqueta "principal"). Alterações:
  - `meeting-ddm-dashboard.html` — novo componente `preencherSetoresMulti` / `renderSetoresMulti` / `toggleSetorMulti` / `setoresSelecionados` (substitui `preencherSetoresSelect`); helper `setoresDaReuniao(r)` = setor principal + `compartilhado_setores` sem repetição; abas de setor e filtro da lista passam a casar por qualquer setor envolvido (antes só o principal); detalhe e `_metaReuniao` listam todos os setores; `compartilhamentoResumo` agora diz "Também envolve: …" para setores e "Compartilhada com: …" para e-mails.
  - `app/api/routes.py` — `POST /api/gravacoes` aceita `setores` (lista): o primeiro vira `setor`, o resto vai para `compartilhado_setores` (JSON). O `INSERT` virou dinâmico e só inclui a coluna se ela existir (`_tem_coluna_reunioes`), então bancos antigos continuam funcionando. Para usuário sem acesso total, se o principal escolhido não for de um setor dele, o backend promove o primeiro setor permitido da seleção em vez de descartar — nada do que foi marcado se perde.
  - `POST /api/reunioes/<id>/metadados` não mudou: já aceitava `compartilhado_setores`; o frontend passou a mandar `setores[0]` como `setor` e o resto na lista.
  - **Modelo de dados sem migration:** a coluna `compartilhado_setores` já existia e a visibilidade por setor compartilhado já estava implementada em `_filtro_visibilidade` e `_pode_acessar_reuniao`. Por isso a reunião aparece para os outros setores sem nenhuma alteração de schema.
  - **Não alterado de propósito:** a estatística `por_setor` continua contando só o setor principal (contar a mesma reunião em N setores inflaria o total), e o filtro `GET /api/reunioes?setor=` continua casando só o principal.
  - Validado: `ast.parse` em `routes.py` e `node --check` no bloco JS do dashboard. **Sem teste em navegador ainda** — falta abrir os dois modais e salvar.
