...
3. **Carregar sem compactação** → selecionar esta pasta (`chrome-extension/`)
...
Nome, setor, formato e local ficam salvos (`chrome.storage.local`) — a partir do segundo uso é só
conferir e enviar.
...
| `options.html/.js` | URL da API + nome padrão + teste de conexão |
...
- `/api/health`, `/api/setores`, `/api/gravacoes`
...
Vazio (default) = qualquer extensão Chrome pode chamar esses 3 endpoints — que já são públicos na
...
2. **Chrome Web Store (não listada):** conta de desenvolvedor (US$5 uma vez), publicar como
   *unlisted*, mandar o link. Atualização automática. ID fixo.
